...
%function config = defaultConfig(edfPath, mriPath, mffPath, sensorCoordinatesPath)
    ...
    config.convert.prefix = 'spmeeg_';

    config.dataPrep.D = prependToFilename(changeToMat(config.convert.D), config.convert.prefix);
    config.dataPrep.prefix = 'DataPrep';
    config.dataPrep.events = config.convert.events;
    config.dataPrep.sensorCoordinatesPath = sensorCoordinatesPath;
    
    config.montage.D = prependToFilename(config.dataPrep.D, config.dataPrep.prefix);
